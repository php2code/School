<?php
  ...
  function smarty_prefilter_i18n($tpl_source, &$smarty) {
    ...
    return preg_replace_callback('/{i18n}(.+?){\/i18n}/', '_compile_lang', $tpl_source);
  }
  ...
  {
    if (
        isset($GLOBALS['_NG_LANGUAGE_']) &&
        is_object($GLOBALS['_NG_LANGUAGE_']) &&
        method_exists($GLOBALS['_NG_LANGUAGE_'], 'getTranslation')
       )
    {
      if ( ! $GLOBALS['_NG_LANGUAGE_']->IsLoaded() ) $GLOBALS['_NG_LANGUAGE_']->loadCurrentTranslationTable();
      return $GLOBALS['_NG_LANGUAGE_']->getTranslation( is_array($key) ? $key[1] : $key );
    }
    else
    {
      return $key;
  ...
  class ngLanguage {
    public $_translationTable;        // currently loaded translation table
    public $_supportedLanguages;      // array of all supported languages
    public $_defaultLocale;           // the default language
    public $_currentLocale;           // currently set locale
    public $_currentLanguage;         // currently loaded language
    public $_languageTable;           // array of language to file associations
    public $_loadedTranslationTables; // array of all loaded translation tables
    ...
    function getTranslation($key) {
      ...
      if (is_array($trans) && is_array($this->_loadedTranslationTables[$this->_currentLanguage])) {
